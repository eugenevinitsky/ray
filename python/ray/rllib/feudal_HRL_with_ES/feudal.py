...
def create_shared_noise(count):
    ...
    seed = 123
    noise = np.random.RandomState(seed).normal(loc=0.0, scale=1.0, size=count).astype(np.float32)
    return noise
...
class FeudalAgent_ES(Agent):
    _agent_name = "Feudal_ES"
    ...
    def _fetch_metrics_from_remote_evaluators(self, noise_table):
        ...
        for metrics in metric_lists:
            ...
            episode_rewards_agents.append(np.mean(episode_rewards_agents_local) if episode_rewards_agents_local else float('nan'))
