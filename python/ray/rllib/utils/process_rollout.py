...
def process_rollout(rollout, reward_filter, gamma, lambda_=1.0, use_gae=True, ADB=False):
    ...
    if use_gae:
        ...
        if ADB:
            ...
            traj["advantages"] = advantages.copy()
...
def process_rollout_Feudal(rollout, reward_filter, gamma, gamma_internal, lambda_=1.0, use_gae=True, tradeoff=0.5, c=10):
    ...
    assert "manager_vf_preds" in rollout and "worker_vf_preds" in rollout, "Values not found!"
    manager_vpred_t = np.stack(rollout["manager_vf_preds"] +
                           [np.array(rollout.last_r)]).squeeze()
    ...
    worker_vpred_t = np.stack(rollout["worker_vf_preds"] +
                               [np.array(rollout.last_r)]).squeeze()
    ...
    for i in range(traj["advantages_manager"].shape[0]):
        ...
        traj["value_targets_worker"][i] = reward_filter(traj["value_targets_worker"][i])
