...
class FeudalLoss(object):
    ...
    def __init__(
        ...
        with tf.variable_scope("VF_Manager"):
            if not config["separate_VF_approximator"]:

                hidden_manager = tf.layers.dense(inputs=self.g , \
                                                     units=config["vf_hidden_size"][0], \
                                                     activation=tf.nn.elu)

                weights_VF_manager = tf.get_variable("weights_VF_manager", (config["vf_hidden_size"][0], 1))
                ...
                conv2_vf_appro = tf.layers.conv2d(inputs=conv1_vf_appro,
                                                          ...
                                                          activation=tf.nn.elu,
                                                          strides=2)
                flattened_filters_vf_appro = tf.reshape(conv2_vf_appro, [-1, np.prod(conv2_vf_appro.get_shape().as_list()[1:])])

                layer = flattened_filters_vf_appro
                ...
                self.value_function_manager = tf.matmul(layer, weights_VF_manager)

        with tf.variable_scope("Loss_Manager"):
        ...
        with tf.variable_scope("VF_WORKER"):
            if not config["separate_VF_approximator"]:
                ...
                weights_VF_worker = tf.get_variable("weights_VF_worker", (config["vf_hidden_size"][0], self.action_dim))
                self.value_function_worker = tf.matmul(hidden_VF_worker, weights_VF_worker)
            else:
                layer = self.carried_g
                for size in config["vf_hidden_size"]:
                    layer = tf.layers.dense(inputs=layer, \
                                            units=size, \
                                            activation=tf.nn.relu)
                ...
                self.value_function_worker = tf.matmul(layer, weights_VF_worker)


        with tf.variable_scope("Loss_Worker"):
        ...
        with tf.variable_scope("Loss_GLOBAL"):
            if not (self.ES):
    ...
    def compute_worker(self, g, z, gsum, observation):
        action, logprobs, vfm, vfw = self.sess.run(
                    self.policy_results,
                    feed_dict={self.observations: [observation], self.carried_g: g, self.carried_z: z, self.carried_gsum: gsum})
