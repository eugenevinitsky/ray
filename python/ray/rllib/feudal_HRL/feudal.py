    ...
    def _train(self):
        ...
        for i in range(config["num_sgd_iter"]):
            ...
            while batch_index < num_batches:
                ...
                batch_loss, batch_manager_policy_loss, batch_manager_vf_loss, batch_worker_policy_loss, \
                batch_worker_vf_loss, batch_kl, batch_entropy = model.run_sgd_minibatch(
                        permutation[batch_index] * model.per_device_batch_size,
                        self.kl_coeff, full_trace,
                        self.file_writer)
                loss.append(batch_loss)
                manager_policy_loss.append(batch_manager_policy_loss)
                manager_vf_loss.append(batch_manager_vf_loss)
                worker_policy_loss.append(batch_worker_policy_loss)
                worker_vf_loss.append(batch_worker_vf_loss)
                kl.append(batch_kl)
                entropy.append(batch_entropy)
                batch_index += 1
            loss = np.mean(loss)
            manager_policy_loss = np.mean(manager_policy_loss)
            manager_vf_loss = np.mean(manager_vf_loss)
            worker_policy_loss = np.mean(worker_policy_loss)
            ...
            print(
                "{:>15}{:15.5e}{:15.5e}{:15.5e}{:15.5e}{:15.5e}".format(
                    i, loss, manager_policy_loss, manager_vf_loss, worker_policy_loss, worker_vf_loss, kl, entropy))
            ...
            if i == config["num_sgd_iter"] - 1:
                ...
                values.extend([
                    ...
                    tf.Summary.Value(
                        ...
                        simple_value=loss),
                    tf.Summary.Value(
                        tag=metric_prefix + "manager_policy_loss",
                        simple_value=manager_policy_loss),
                    tf.Summary.Value(
                        tag=metric_prefix + "manager_vf_loss",
                        simple_value=manager_vf_loss),
                    tf.Summary.Value(
                        tag=metric_prefix + "worker_policy_loss",
                        simple_value=worker_policy_loss),
                    tf.Summary.Value(
